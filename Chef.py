...
class ChefInt(CTk.CTk):
    def __init__(self):
        ...
        self.chefInt_frame.grid(row=0, column=0)

        self.logo_text = CTk.CTkLabel(master=self.chefInt_frame, width=1400, height=80, font=("Courier", 20),
                                      text_color="black",
                                      text="Afflatus")
        self.logo_text.grid(row=0, column=0, sticky="n")

        self.recepies_frame = CTk.CTkFrame(master=self.chefInt_frame, fg_color="white", width=1400, height=820)
        ...
        self.menu_frame = CTk.CTkFrame(master=self.chefInt_frame, width=1400, height=100)
        self.menu_frame.grid(row=2, column=0)
        menu_list = ['Recommendation', 'Search', 'My Recipes', 'Liked', 'Watched', 'Reviews', 'Profile']
        for r in range(7):
